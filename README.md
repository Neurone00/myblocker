...
Adbrella runs a **local VPN that captures only DNS**. Every app's lookups pass through it; names on the blocklists get an instant empty answer, so the ad or tracker never loads. Everything else is forwarded, encrypted (DNS-over-HTTPS to Quad9 by default), to the resolver you choose. Real traffic never enters the tunnel at all, so there is no speed or battery cost beyond the DNS work itself.
...
- **Tidy web pages (Samsung Internet only)**: Adbrella registers as a **Samsung Internet content blocker** (the same extension API AdGuard Content Blocker and Adblock Plus use) and supplies EasyList, whose element-hiding rules remove the empty ad boxes DNS blocking leaves behind and let pages reflow. One-time step: Samsung Internet › Settings › Extensions › Content blockers › switch on Adbrella (Settings › Tidy web pages opens that page). No certificate and no traffic interception are involved. Other browsers block their own ad boxes and should be left to it: Brave has Shields, Firefox takes uBlock Origin. Chrome supports neither, so ads there are blocked at the DNS level but their empty boxes remain.
...
- **For a phone a child uses**: family-filtering resolvers (Cloudflare for Families, AdGuard Family Protection, CleanBrowsing Family — the last two also force safe search on Google, Bing and YouTube) and two opt-in content lists (adult, and adult plus gambling). Both work at the DNS level, so one setting covers every app and browser on the phone. Neither can judge a particular ad or tell one video from another on YouTube or TikTok, where content and ads share a domain, and anyone who can open the app can switch it off — it belongs underneath Google Family Link, not instead of it.
...
- **State at a glance**: the status bar icon *is* the state — an open umbrella while you are covered, a furled one while protection is off or paused for Android Auto — and the Quick Settings tile follows it. Both icons are generated from the same dome geometry the home card animates, so it is one umbrella everywhere.
...
- **Empty ad boxes outside Samsung Internet.** DNS blocking stops the ad loading but leaves the space the page reserved for it. Removing that needs rules running inside the browser. A TLS-intercepting proxy was built for this and removed again: it requires a certificate Android only lets you install by hand, and it broke browsing twice before it ever removed a box. Brave's Shields and Firefox's uBlock Origin do the same job properly, inside the browser, with nothing to install.
...
app/src/main/java/com/neurone/myblocker/
  ...
  web/       WebFilters (EasyList refresh, Samsung Internet notify) · CosmeticRules · WebFilterProvider (content-blocker provider)
