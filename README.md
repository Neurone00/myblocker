...
- **Deep clean (beta, Advanced)**: routes all traffic through Adbrella's own userspace TCP/UDP relay instead of only DNS, and creates a per-phone certificate authority (a small X.509 encoder, no BouncyCastle) whose public certificate you install from Settings. Phases 1 and 2 of in-page filtering for Chrome; page rewriting itself is the next phase.
