...
- **Deep clean (beta, Advanced)**: tidies pages in Chrome, Brave, Samsung Internet and similar browsers, the way AdGuard's paid app does. All traffic is routed through Adbrella's userspace TCP/UDP relay; browser connections go to a local proxy that terminates TLS with a per-phone certificate authority (a small X.509 encoder, no BouncyCastle; you install only the public certificate, by hand in Settings — from Android 11 the system refuses any app-initiated CA install, so Advanced saves the file to Downloads and opens Settings for you, and the tunnel picks the certificate up by itself once it lands), verifies the real server against the system trust store, relays HTTP/1.1 and adds two stylesheet links to HTML so EasyList's element-hiding rules remove the empty ad boxes, plus a small in-page collapser that hides leftover placeholders (grey "ADV" boxes, wrappers left empty around a blocked slot, slots inserted later while scrolling) and gives a box back if real content turns up in it; it runs against a synthetic news page in CI (`tools/collapser-test`). CSP headers are extended, not removed; Alt-Svc is stripped so intercepted origins fall back to HTTP/1.1. Dropping QUIC outright would tidy more pages but is off by default and stays a separate switch: UDP carries no reliable per-connection app identity, so it black-holes QUIC for every app, and a silent drop gives a browser nothing to fail over from — it waits out its own timeouts and the page hangs half-loaded. A site that cannot be intercepted at all (our certificate refused, or its TLS not negotiating) is added to a troubled list and tunnelled raw from then on, so tidying can never leave a page broken. Only browsers are ever intercepted; every other app, including anything that pins certificates (Instagram, YouTube, banking), passes through untouched. Within browsers, a built-in list of certificate-pinned hosts (Google, Apple, Microsoft, Meta, payment and Samsung account domains) plus any you add is tunnelled raw by SNI so pinning still holds, and the interception path is covered by an end-to-end test that stands up a real TLS origin and checks the injected result.
