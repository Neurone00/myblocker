...
- **Android Auto aware**: Android Auto refuses to start with any VPN active (error 21) and detects the VPN itself, so a bypass cannot help. Android Auto also checks *before* it ever reports being connected, so Adbrella watches for the car instead — it pauses protection the moment the phone joins the car over Bluetooth, enters car mode, or is plugged in over USB in accessory mode (which wired Android Auto uses and a plain charger does not), all of which happen before Android Auto starts, and resumes on disconnect (Settings → Pause for Android Auto).
